...
let lazyManifest = {
  _comment: 'manifest was never reassigned, this should never occur',
  _invalid: true
};
...
export default function clientRouter() {
  ...
  router.get('/*', async (req, res) => {
    ...
    if (lazyManifest._invalid) lazyManifest = await parseManifest();
    ...
    res.render(
      ...
      (err, html) => {
        if (err) {
          ...
          expressLog(
            `express gave the following information: ${JSON.stringify(err)}`
          );
          res.status(500).send();
        } else {
          res.send(html);
...
async function parseManifest() {
  ...
  return fs
    ...
    .catch((err) => {
      expressLog(
        `fs read gave an error, here's what we know: ${JSON.stringify(err)}`
      );
      return `
            {
                "_comment": "fs read gave an error, here's what we know: ${JSON.stringify(
                  err
                )},
                "_invalid": true
            }`;
