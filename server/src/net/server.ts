...
import {expressLog, consoleColors} from "../util.js";

const port = process.env.PORT || 8080;
const environment = process.env.NODE_ENV || "development";
const rootDir = process.env.ROOT_DIR;
const suppressSanity = process.env.SUPPRESS_SANITY;
const publicPath = path.join(rootDir || path.resolve('./dist'), "client", "public");
    ...
    let internal = app.listen(port, () => {
        ...
        if (environment === "production") { //Production warnings
            ...
            if (rootDir == null)
                ...
                expressLog(`Root dir set to '${rootDir}'. Assets will be served from here rather than the current working directory...`)
        }
